...
y = 100
import os
os.environ['SDL_VIDEO_WINDOW_POS'] = f'{x}, {y}'  # Thanks to https://stackoverflow.com/questions/68552586/pygame-zero-window-positioning
...
run_images = ['run__000', 'run__001', 'run__002', 'run__003', 'run__004', 'run__005', 'run__006', 'run__008', 'run__009', ]
...
def update():
    ...
    obstacles_timeout += 1
    timeout_number = random.randint(50, 100)
    if obstacles_timeout > timeout_number:
        actor = Actor('cactus')
        actor.x = 850
        actor.y = 430
    ...
    for actor in obstacles:
        actor.x -= 8
        if actor.x < -50 and game_over == False:
